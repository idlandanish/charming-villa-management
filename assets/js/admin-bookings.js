...
function badge(status){
  ...
  // status is internal code, safe to print. map lookup ensures validity.
  return `<span class="pill ${map[status]||"muted"}">${status.replaceAll("_"," ")}</span>`;
}
...
async function refresh(){
  ...
  const tbody = document.getElementById("rows");
  // USE esc() HERE FOR USER INPUTS
  tbody.innerHTML = rows.map(b=>`
    <tr>
      <td>#${esc(b.id)}</td>
      <td><b>${esc(b.unit_name)}</b><div class="muted small">${esc(b.location)}</div></td>
      <td>${esc(b.check_in)} → ${esc(b.check_out)}</td>
      <td>${esc(b.customer_name)}<div class="muted small">${esc(b.customer_phone||"")}</div></td>
      <td>${esc(b.channel||"—")}</td>
      <td>${badge(b.status)}</td>
      <td>${money(b.deposit_amount)}<div class="muted small">${b.deposit_paid==1?"Paid":"Not paid"}</div></td>
      <td><button class="btn" data-edit="${esc(b.id)}" type="button">Edit</button></td>
    </tr>
  `).join("");
