...
function cardHTML(u, available, checkIn, checkOut){
  ...
  const viewBtn = u.link
    ? `<a class="btn" href="${u.link}">View unit</a>`
    : `<a class="btn" href="./properties.html">View unit</a>`;

  return `
  <div class="card">
    <div class="pcard-title">${u.display_name}</div>
    <div class="pcard-meta">${u.location} • ${u.type}</div>
...
async function init(){
  ...
  const bookingsRes = await fetchJSON("./api/bookings_public_blocked.php");
  ...
  qs("checkIn").value = fmt(today);
  qs("checkOut").value = fmt(t1);
  ...
  qs("btnCheck").addEventListener("click", run);
  ["loc","unit","checkIn","checkOut"].forEach(id=>qs(id).addEventListener("change", run));
  run();
}
